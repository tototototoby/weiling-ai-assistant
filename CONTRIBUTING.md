...
微Link项目方拥有版权的代码采用 `AGPL-3.0-only` 社区许可证，并可能通过单独协议提供商业许可证。为了让项目能够持续维护这套双许可证结构，实质性外部贡献只有在贡献者接受 [Contributor License Agreement](CLA.md) 后才能合并。

提交贡献表示你希望该贡献被项目评估；只有在 Pull Request 中明确勾选 CLA 声明，才表示你授予 CLA 中约定的权利。普通的 `Signed-off-by` 或 DCO 不替代本项目 CLA。

如果贡献属于雇主、客户或其他主体，请先确认你有权提交并授权。第三方代码、素材、模板、模型输出或许可证必须在 PR 中清楚披露；项目商业许可证不会自动覆盖这些内容。
