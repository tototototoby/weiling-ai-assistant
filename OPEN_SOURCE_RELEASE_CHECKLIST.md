...
- [x] 保留 `v0.1.0-beta.1` 的历史 MIT 授权，不删除、不重写同名 Tag。
- [x] 从 `v0.2.0-beta.1` 起采用 `AGPL-3.0-only` 社区许可证与单独商业授权。
- [x] 新增 `NOTICE`、商业授权说明和 CLA，并保留 WeClaws MIT 许可副本。
...
- [x] 用户确认已在其他环境验证 Docker/Compose 部署；当前源码部署不依赖尚未发布的私有应用镜像。
...
- [x] 用户确认已在其他环境用测试账号完成微信扫码、消息收发和二维码分享验收。
- [x] 用户确认已在其他环境完成企业微信连接、首次绑定、收发和允许范围内的主动投递验收。
- [x] 用户确认已在其他环境完成飞书私聊、群聊、附件和权限验收，并核对群聊上下文边界。
- [x] 用户确认已完成“同一 Bot 从微信切换到飞书/Web 继续任务”的真实跨端测试。
- [x] 用户确认已完成“客户端离开后，服务端任务继续并返回结果”的真实托管测试。
- [x] 用户确认已完成主动提醒的真实投递测试；飞书仍不写成中央主动提醒的默认投递渠道。
...
- FastAgent、Browserless、WeClaws 和其他第三方内容继续使用各自许可；根目录 AGPL 与商业授权均不覆盖这些组件。
