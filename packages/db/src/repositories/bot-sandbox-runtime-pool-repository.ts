...
import { botSandboxRuntimePools } from '../schema/bot-sandbox-runtime-pools';

type Db = BetterSQLite3Database<typeof schema>;
type BotSandboxRuntimePoolRow = typeof botSandboxRuntimePools.$inferSelect;

const API_KEY_BYTES = 32;
const PROXY_PORTS_PER_WORKER = 2;
const SAFE_PATH_SEGMENT_PATTERN = /^[A-Za-z0-9_-]+$/;

export interface BotSandboxRuntimePoolRecord {
  apiKey: string;
...
export interface UpdateBotSandboxRuntimePoolInput {
  defaultAllowRead?: string[];
  defaultAllowWrite?: string[];
  defaultDeniedDomains?: string[];
  defaultDenyRead?: string[];
...
export class BotSandboxRuntimePoolRepository {
  constructor(private readonly db: Db) {}

  async ensureForBot(input: EnsureBotSandboxRuntimePoolInput): Promise<BotSandboxRuntimePoolRecord> {
    return this.db.transaction((tx) => {
      const existing = tx.select()
        .from(botSandboxRuntimePools)
        .where(eq(botSandboxRuntimePools.botInstanceId, input.botInstanceId))
        .get();
      ...
      assertSafePathSegment(input.botInstanceId, 'botInstanceId');
      validateCapacity({
      ...
      const existingPools = tx.select()
        .from(botSandboxRuntimePools)
        .orderBy(asc(botSandboxRuntimePools.port))
        .all();
      ...
      tx.insert(botSandboxRuntimePools).values({
        ...
        updatedAt: now,
        workspaceBasePath: posix.join(input.defaults.workspaceBaseRoot, input.botInstanceId),
      }).run();
      ...
      if (!created) {
        throw new Error('Failed to create Bot SRT pool.');
      }
  ...
  async findByBotInstanceId(botInstanceId: string): Promise<BotSandboxRuntimePoolRecord | null> {
    const row = this.db.select()
      .from(botSandboxRuntimePools)
      .where(eq(botSandboxRuntimePools.botInstanceId, botInstanceId))
      .get();

    return row ? mapRow(row) : null;
  }

  async listAll(): Promise<BotSandboxRuntimePoolRecord[]> {
    return this.db.select()
      .from(botSandboxRuntimePools)
      .orderBy(asc(botSandboxRuntimePools.botInstanceId))
      .all()
      .map(mapRow);
  }

  async updateByBotInstanceId(
    botInstanceId: string,
    input: UpdateBotSandboxRuntimePoolInput,
  ): Promise<BotSandboxRuntimePoolRecord | null> {
    return this.db.transaction((tx) => {
      ...
      validatePositiveInteger(next.healthCheckIntervalMs, 'Bot SRT pool healthCheckIntervalMs');
      validatePositiveInteger(next.maxConcurrentInit, 'Bot SRT pool maxConcurrentInit');
      validatePositiveInteger(next.port, 'Bot SRT pool port');
      validatePositiveInteger(next.sessionTimeoutMs, 'Bot SRT pool sessionTimeoutMs');
      validateCapacity(next);

      const otherPools = tx.select()
        .from(botSandboxRuntimePools)
        .where(ne(botSandboxRuntimePools.botInstanceId, botInstanceId))
        .all();

      if (otherPools.some((pool) => pool.port === next.port)) {
        throw new Error('Bot SRT pool port is already used by another pool.');
      }

      if (otherPools.some((pool) => (
        rangesOverlap(pool.portRangeStart, pool.portRangeEnd, next.portRangeStart, next.portRangeEnd)
      ))) {
        throw new Error('Bot SRT pool proxy port range overlaps another pool.');
      }

      tx.update(botSandboxRuntimePools)
        .set({
  ...
  async requestRestart(
    ...
    this.db.update(botSandboxRuntimePools)
      .set({
...
function allocatePort(portBase: number, existingPools: BotSandboxRuntimePoolRow[]): number {
...
function allocateProxyPortRange(
  defaults: SandboxRuntimePoolDefaults,
  existingPools: BotSandboxRuntimePoolRow[],
): number {
...
function mapRow(row: BotSandboxRuntimePoolRow): BotSandboxRuntimePoolRecord {
  return {
    apiKey: row.apiKey,
...
function parseStringArray(raw: string): string[] {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== 'string')) {
    throw new Error('Bot SRT pool JSON array field is invalid.');
  }
...
function validateCapacity(input: {
  ...
  validatePositiveInteger(input.poolSize, 'Bot SRT pool poolSize');
  validatePositiveInteger(input.minReadyProcesses, 'Bot SRT pool minReadyProcesses');
  validatePositiveInteger(input.portRangeStart, 'Bot SRT pool portRangeStart');
  validatePositiveInteger(input.portRangeEnd, 'Bot SRT pool portRangeEnd');

  if (input.minReadyProcesses > input.poolSize) {
    throw new Error('Bot SRT pool minReadyProcesses must be <= poolSize.');
  }

  if (input.portRangeEnd < input.portRangeStart) {
    throw new Error('Bot SRT pool portRangeEnd must be >= portRangeStart.');
  }

  if (input.portRangeEnd - input.portRangeStart + 1 < input.poolSize * PROXY_PORTS_PER_WORKER) {
    throw new Error('Bot SRT pool proxy port range must cover poolSize * 2 ports.');
  }
}
